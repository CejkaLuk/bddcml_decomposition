...
! Subdomain structure
      ...
      ! .mtx file variables - 2x time vars; string for time in ms; file name
      logical :: write_mtx_to_file = .false.
      integer, dimension(8):: v
      integer(8) :: timems
      character(len=70) :: file_id
      ...
      if (sub%is_explicit_schur_prepared) then
         ...
         if (write_mtx_to_file) then
            ! Matrix: sub%aaug_dense
            write(*,*) "----------------------------------------"
            !do, i=1,m
            !   do, j=1,n
            !      write (*,*) A(i,j)
            ...
            write (*,'(a)') '[START] - Writing matrix to file...'

            num_nz = 0
            do, i=1,sub%laaug_dense1
                  do, j=1,sub%laaug_dense2
                     if ( .not. sub%aaug_dense(i,j).eq.0) then
                           num_nz = num_nz + 1
                     end if
                  enddo
            enddo
            ...
            write(file_id, '(i0)') timems
            ! Construct the filename:
            file_name = '../p_on_cube_matrices/matrices/poc-8_4_2-' // trim(adjustl(file_id)) // '.mtx'
            ! output data into a file
            open(1, file = trim(file_name), status = 'new')
            write(1,*)'%%MatrixMarket matrix coordinate real general'
            write(1,*) sub%laaug_dense1,sub%laaug_dense2,num_nz
            do, i=1,sub%laaug_dense1
               do, j=1,sub%laaug_dense2
                  if ( .not. sub%aaug_dense(i,j).eq.0) then
                        write(1,*) i,j,sub%aaug_dense(i,j)
                  end if
               enddo
            enddo
            close(1)
            write (*,'(a)') '[DONE] - Writing matrix to file...'
            write(*,*) "----------------------------------------"
         end if
